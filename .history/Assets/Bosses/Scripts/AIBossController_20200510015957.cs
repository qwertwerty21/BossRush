...
public abstract class AIBossController : MonoBehaviour
{
  ...
  virtual protected void Update()
  {
    ...
    float magnitude = Mathf.Abs(m_RigidBody.velocity.magnitude);
    Debug.Log("magnitude" + m_Time);
  ...
  {
    if (otherCollider.gameObject.tag == "Ground" && (m_Time > 1f))
    {
      Debug.Log("COllEIDERFDFD");
      if (!m_IsNavMeshAgentUpdating)
      {
        m_Time = 0f;
