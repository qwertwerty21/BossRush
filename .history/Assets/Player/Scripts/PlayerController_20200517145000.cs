...
public class PlayerController : MonoBehaviour
{
  ...
  public void MoveTowardsClosestTarget()
  {
    if (m_ClosestTargetTransform)
    {
      ...
      Vector3 cameraPosition = new Vector3(m_ClosestTargetTransform.position.x, Mathf.Clamp(m_Camera.position.y, .4f, .6f), m_ClosestTargetTransform.position.z);

      transform.LookAt(characterPosition);
      m_Camera.LookAt(cameraPosition);
