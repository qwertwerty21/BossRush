...
public class PlayerController : MonoBehaviour
{
  ...
  private void GetClosestTarget()
  {
    ...
    RaycastHit[] hits = Physics.SphereCastAll(m_Camera.transform.position, m_AutoTargetingRadius, m_Camera.transform.forward, m_AutoTargetingRange, m_LayerMask);
    float closestDistance = Mathf.Infinity;
    Transform closestTargetTransform = null;
    foreach (RaycastHit hit in hits)
    {
      Target target = hit.gameObject.GetComponent<Target>();
      if (target)
      {
        // get distance 
        float distance = Vector3.Distance(target.transform.position, transform.position);
