...
public class Sword : MonoBehaviour
{
  ...
  {
    // if (()
    Boolean wasAttacking = m_Animator.GetBool("isAttacking");
    if (m_Animator.GetBool("isAttacking") && (Time.time - m_AttackLastTapped) > 3f)
    {
      Debug.Log("Seting to false isAttacking");
      m_Animator.SetBool("isAttacking", false);
    }
    // m_Animator.SetBool("isAttacking", (Time.time - m_AttackLastTapped) < 120f);
    // } e
    if (Input.GetButtonDown("Fire1"))
    {

      // StartCoroutine(SwordAttack());
      m_AttackLastTapped = Time.time;
      m_Animator.SetBool("isAttacking", true);
      m_Animator.SetTrigger("lightSwordAttack");
      ...
      Debug.Log("IS ATTACKING" + m_Animator.GetBool("isAttacking"));
